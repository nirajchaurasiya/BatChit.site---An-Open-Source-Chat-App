...
const sendActivationEmail = asyncHandler(async (req, res) => {
  try {
    ...
    return res.status(200).json(new ApiResponse(200, [], "Email sent!", 6001));
  } catch (error) {
...
const checkActivationEmail = asyncHandler(async (req, res) => {
  try {
    ...
    res.status(500).sendFile(path.join(__dirname, "/html/expired-token.html"));
  }
});
...
const searchUser = asyncHandler(async (req, res) => {
  ...
  const result = await User.aggregate([
    {
      $match: {
        $or: [
          ...
          {
            email: { $in: keywords.map((keyword) => new RegExp(keyword, "i")) },
  ...
  return res
    .status(200)
    .json(
      new ApiResponse(200, result, "Searched user retrieved successfully", 6001)
    );
});
